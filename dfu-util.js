                ...
                reader.onload = async function() {
                    ...
                    if (device && firmwareFile != null) {
                        ...
                        await device.do_download(transferSize, firmwareFile, manifestationTolerant).then(
                            () => {
                                logInfo("Your CAN has been updated! Return the boot jumper to its normal position, if applicable. Unplug/replug the device to start using the new firmware.");
                                setLogContext(null);
            ...
            dfu.findAllDfuInterfaces().then(
                async dfu_devices => {
                    ...
                    if (matching_devices.length == 0) {
                        statusDisplay.textContent = 'Please plug in your CAN in boot mode'; // EMZ suppress this message 'No device found on auto.';
                    } else {
                        if (matching_devices.length == 1) {
                            ...
                            statusDisplay.textContent = "Detected CAN device";
                        }
